...
/** A shelf of singles, all from one buy-in, for the batch rules. */
function shelf(count: number, tradeIn = "trade_bulk"): ItemRecord[] {
  const made: ItemRecord[] = []
  for (let index = 0; index < count; index += 1) {
    const body = index.toString(32).toUpperCase().padStart(5, "0")
    made.push({
      id: `item_bulk_${index}`,
...
describe("what a selector takes", () => {
  ...
  it("takes a location, a kind or a game on its own", () => {
    ...
    expect(queueBatch({ location: "loc_binder_a" }).queued).toBe(2)
    expect(queueBatch({ kind: "single", include_queued: true }).queued).toBe(2)
    expect(queueBatch({ game: "game_pokemon", include_queued: true }).queued).toBe(2)
  })

  it("passes over an item that already has a label waiting", () => {
    itemStore().push(...shelf(2))
    expect(queueBatch({ location: "loc_binder_a" })).toMatchObject({
      queued: 2,
      skipped: 0,
    })
    expect(queueBatch({ location: "loc_binder_a" })).toMatchObject({
  ...
  it("carries the copies and the size the sheet asked for", () => {
    ...
    queueBatch({ location: "loc_binder_a", copies: 3, template: "sleeve_25x15" })
...
describe("claiming and reporting", () => {
  it("gives one device the oldest jobs its roll can print", () => {
    ...
    queueBatch({ location: "loc_binder_a" })
