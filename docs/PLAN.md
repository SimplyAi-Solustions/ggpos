...
2. **Scan** (one tap, and the global wedge listener): decodes item SKUs (`GGS…`), customer codes (`GGC…`), reward codes (`GGV…`) and EANs, and routes by prefix. Price-check mode shows market value, our offer band and our stock for any card without creating anything.
