...
One row per UTC calendar day, built straight from the ledgers and rows - never from a cached balance (CLAUDE.md's "Pricing" section states this rule for offers; it applies here to every figure). Every date bound in this package is a UTC day, compared in PocketBase's own stored form (`YYYY-MM-DD HH:MM:SS.sssZ`, a space rather than a "T" - `pb_hooks/crons.pb.js`'s existing `pbDate()` and `exports.pb.js`'s `acquired_at` range do the same for the same reason).

- `sales_count` / `sales_total_by_payment`: every `sales` row created that day, counted and summed by `payment`: `sumup_card`, `cash`, `store_credit`, `points`, `mixed`, and **`none`** - an eBay order import (`sales.channel = "ebay"`, `docs/api-contract.md`'s "Phase 4: exports, imports and SumUp") leaves `payment` blank because eBay took the money, and that is its own bucket, never folded into `mixed` (which means the shop itself split one sale across methods). `sales_count` counts every sale regardless of channel or payment, eBay imports included.
...
- `items_out`: `sale_lines` rows created that day, summed as `qty - refunded_qty` (only where positive) - net of every refund on file at build time, not only refunds that happened the same day. Rebuilding an old day's row therefore nets off a later refund against the day the sale was made, which is the point of "net of refunds".
...
- `returning_customers`: of the customers with a sale or a completed trade-in that day, how many also have an *earlier* sale or completed trade-in (before that day's start) - a repeat, not a first visit.
...
- **`channels`** - in-store vs eBay. `table` rows: `{ key, label, revenue, count }`, keyed on **`sales.channel`** (`"counter"` when blank, `"ebay"` when set - never on `items.ebay_listing_id`, which is about listings, not sales). `totals`: `{ revenue, count, by_channel, listing_ages, items_ended }`. `listing_ages` is `{ item_id, sku, title, days_listed }` for every item currently `status = "listed_ebay"`. `items_ended` counts items with a non-empty `ebay_listing_id` whose `status` is no longer `listed_ebay` and whose `updated` falls in range - a proxy for "this listing ended", since there is no dedicated "ended" status.
...
- **The channels report keys off `sales.channel`, never `items.ebay_listing_id`** - the latter is about a listing's own lifecycle (its age, whether it has ended), which stays a separate figure on the same report rather than standing in for which channel a sale went through.
