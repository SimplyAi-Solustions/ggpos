...
- `stock_value_cost` / `stock_value_market`: **point in time**, not a historical reconstruction of what was held at the end of that specific day. Every `items` row currently `in_stock`, `reserved` or `listed_ebay` is summed as `cost * qty` for cost, and for market, `qty` times the latest `price_snapshots.gbp_market` for that item's card or retro title when one exists (whatever its source or finish/completeness - the newest figure on file, the same way the stock report's price movers read it), else `market_at_intake`. Because this reads current stock rather than a snapshot frozen on the day, a rebuild of a much older date shows today's stock levels, not that day's - accurate for the ordinary case (the nightly cron builds *yesterday*, a few hours after midnight) and worth knowing for a manual backfill of an old range.
...
`POST /api/vault/stats/rebuild?from=YYYY-MM-DD&to=YYYY-MM-DD` (**admin**) upserts one row per day in the range, matched by date so a second rebuild of the same range updates the same rows rather than duplicating them. Bounded to 400 days, same refusal as the reports routes below. Response: `{ "from", "to", "days": 7 }`. Audited: `collection: "daily_stats"`, `meta: { from, to, days }`.

A cron named `stats` builds yesterday's row at 00:30 UTC. `POST /api/crons/stats` (PocketBase's own "run this job now" route, superuser only) runs it on demand.
...
**staff**; `compliance` and the separate `audit.csv` export below are **admin**. `from` and `to` are required; missing or malformed dates refuse with 400 `"Pick a date range. Both from and to are needed, as YYYY-MM-DD."`; `from` after `to` refuses with 400 `"The from date is after the to date. Swap them over."`; a range over 400 days refuses with 400 `"Pick a range of up to 400 days."`. `group` defaults to `day` and falls back to it for any other value; `week` labels are always that week's **Monday**; `month` labels are `YYYY-MM`. `by` is report-specific (listed per key below) and falls back to that report's first-listed dimension when omitted or unrecognised. `compare` is `none` (default) or `previous`: the immediately preceding period of the same length, with its own `totals` recomputed the same way.
...
`series` and `totals` for `sales`, `buyins`, `customers` and `loyalty` read `daily_stats` per day in range (falling back to a fresh, unsaved computation for any day the nightly cron has not reached yet, so a report is never wrong just because the build is behind); `table` (the `by=<dimension>` breakdown) and every report-specific figure not on `daily_stats` come from direct queries, bounded to the same range. `stock`, `channels`, `margin`'s markdowns, `cash` and `compliance` query directly throughout, since none of their shapes map onto a single day row.

Every report also serves `GET /api/vault/reports/:key.csv?...` (the same parameters) as a CSV of its `table`, `Content-Disposition: attachment`, a header row from the table's column labels, and the same formula-injection guard `pb_hooks/lib/vaultutil.js`'s `csvCell` uses for the stock book (a cell starting with `=`, `+`, `-`, `@`, a tab or a carriage return is prefixed with a quote and quoted).

Keys:

- **`sales`** - revenue by period; `by`: `game` (default) | `kind` | `staff` | `payment`. `table` rows: `{ key, label, revenue, count }`. `series` values: `{ revenue, count, avg_basket }`. `totals`: `{ revenue, count, avg_basket, heatmap }` - `heatmap` is 7 arrays of 24 integers, sale counts by hour, **index 0 is Monday, 6 is Sunday**, hours 0 to 23 UTC. `by=payment`'s bucket for a blank `payment` (an eBay-import sale) is labelled `(none)`, matching `sales_total_by_payment`'s `none` above.
- **`buyins`** - spend by period; `by`: `game` (default) | `staff`. `table` rows: `{ key, label, spend, count, avg_offer_pct }` (`avg_offer_pct` is populated for `by=game`, since offer percent is a line-level, per-card figure; it reads 0 on `by=staff` rows - the headline figure below covers all trade-ins regardless of dimension). `series` values: `{ spend, count }`. `totals`: `{ spend, count, avg_offer_pct, cash, credit, items_bought, items_sold, sell_through_ratio, top_sellers }` - `avg_offer_pct` is `offer_price / market_price * 100` averaged over every accepted line with `market_price > 0`; `items_bought` / `items_sold` are a running conversion rate for stock this shop has taken in (not bounded to items bought *and* sold within the same range); `top_sellers` is up to 10 `{ customer, name, count, spend }`, most trade-ins first.
- **`margin`** - gross margin; `by`: `game` (default) | `kind` | `staff`. Sale price is net of the sale-level discount and of every refund on file, through the same shared `saleline` evaluator the stock book and the refund route use (`pb_hooks/lib/shared/saleline.js`), so this can never disagree with either about what a line actually made. `table` rows: `{ key, label, revenue, cost, margin, margin_pct }`. `series` values: `{ revenue, cost, margin }`. `totals`: `{ revenue, cost, margin, margin_pct, vat_estimate, markdown_count, markdown_value }`. `vat_estimate` is the margin-scheme VAT estimate: one sixth of the *positive* margin on lines whose item carries `tax_scheme: "margin"` (the VAT fraction of the standard 20 percent rate), zero whenever `settings.vat_registered` is off - an estimate for the reports screen, not a return. `markdown_count` / `markdown_value` are a proxy: every `items` row (any status) whose current `price` sits below its `market_at_intake`, valued at the difference - a proxy for "this was marked down", not a log of every price change actually made.
- **`stock`** - value at cost vs market, ageing, sell-through, dead stock and price movers. Everything except sell-through is **point in time** (as stock stands when the report runs), the same caveat as `daily_stats.stock_value_*` above. `by`: `game` (default) | `set`, for sell-through only. The primary `table` (and its CSV) is **price movers**: `{ item_id, sku, title, market_at_intake, latest_market, pct_change }` for every held item (`in_stock`, `reserved` or `listed_ebay`) whose card or retro title has a `price_snapshots` row and whose latest `gbp_market` differs from `market_at_intake` by more than 15 percent either way, sorted by the size of the move, `item_id` included so the app can offer a one-click reprice. `totals`: `{ value_cost, value_market, unrealised_gain, ageing_buckets, sell_through, dead_stock, price_movers_count }`. `ageing_buckets` is always 4 rows, `{ bucket, min, max, count, value_cost, value_market }` for `"0-30"`, `"31-90"`, `"91-180"`, `"180+"` days since `acquired_at` (`max` is `null`, never a value that cannot round-trip through JSON, on the open-ended `"180+"` bucket). `sell_through` is `{ key, label, acquired, sold, rate }` per game or set: **stock rows**, not units - a sealed line of qty *n* is one row, and since a sale decrements `items.qty` in place, a partly-sold multi-quantity row's original quantity cannot be recovered from its current state, so a row only counts as sold once it has sold out completely (`status = "sold"`). `dead_stock` is `{ item_id, sku, title, days_held, cost, market }` for every held item over 180 days since `acquired_at` - the `"180+"` ageing bucket's own rows.
- **`channels`** - in-store vs eBay. `table` rows: `{ key, label, revenue, count }`, keyed on **`sales.channel`** (`"counter"` when blank, `"ebay"` when set - a *sale's* channel, never an item's own eBay fields). `totals`: `{ revenue, count, by_channel, listing_ages, items_ended }`. `listing_ages` is `{ item_id, sku, title, days_listed }` for every item currently `status = "listed_ebay"`. `items_ended` counts items whose `status` is no longer `listed_ebay`, whose `updated` falls in range, and that were on eBay at all - `ebay_listing_id != "" || ebay_sku != ""`, not `ebay_listing_id` alone: nothing in this backend writes `ebay_listing_id` (docs/PLAN.md's eBay round trip is planned, not built), and a Card Uploader listing carries `ebay_sku` (docs/api-contract.md's "Phase 4: exports, imports and SumUp"). A proxy either way, since there is no dedicated "ended" status.
- **`customers`** - new vs returning; top by spend and by trade-in; credit liability; want-list demand. `table` rows mirror the top-by-spend list: `{ customer, label, amount }`. `series` values: `{ new, returning }`. `totals`: `{ new, returning, top_by_spend, top_by_trade_in, credit_liability, want_list_demand }`, the first two up to 10 `{ customer, name, amount }` each. `credit_liability` is the sum of every `credit_ledger.amount` created on or before the end of the range - never `customer_private.credit_balance`'s cache - which is also each customer's own recomputed balance summed across every customer, for free. `want_list_demand` is `{ key, card_id, label, count, in_stock }` for open `want_list` rows grouped by card (or by free text when there is no card), most-wanted first, `in_stock` true when the shop currently holds a matching item.
- **`loyalty`** - points issued/redeemed, tiers, perks, rewards, referrals, programme cost. `table` rows are the tier distribution: `{ tier, label, count }` (plus a `{ tier: "", label: "No tier", count }` row when any `customer_private` row has none). `series` values: `{ points_earned, points_redeemed }`. `totals`: `{ points_earned, points_redeemed, tier_distribution, perk_usage, reward_take_up, referrals: { total, earned }, programme_cost_pct }`. `perk_usage` is `{ perk_type, used_count }` summed over `perk_usage` rows created in range; `reward_take_up` is `{ reward, label, count, points_spent }` per reward, from `reward_redemptions` created in range. `programme_cost_pct` values the points redeemed in range at the programme's `points_per_pound_redemption` (the shared `pointsToPence`, `pb_hooks/lib/shared/loyalty.js`) over sales revenue for the same range.
...
`saved_reports` rows (`owner`, `report_key`, `filters` json, `name`, `schedule`: `none` | `weekly` | `monthly`, `recipients`) are ordinary collection-API records, staff create/read/update/delete (the seeded rule from `1789819560_ops_collections.js`, unchanged). `filters` holds `{ by, group }` only - `from` and `to` are never read from a saved row for a scheduled send; the period is always computed fresh from the schedule (see below), so a saved weekly report always covers the week that just ended, never a stale literal date. `recipients` is an array of email addresses; when empty, the send falls back to the owner staff member's own email, and is skipped (logged, not failed) when neither exists.
...
Each send emails the report's `totals` as plain text (one line per numeric figure, money through the shared `formatGBP`) with its `table` attached as a CSV, through the app mail client the way `pb_hooks/lib/receipts.js` does, honouring `settings.email.test_mode` (logs `"[scheduled_reports:test_mode] would email ... (not sent)"` instead of sending). Every send, sent or logged, is audited by the saved report's id alone: `collection: "saved_reports"`, `record: <saved_reports id>`, `meta: { sent, test_mode }`.
...
- **UTC day boundaries throughout**, compared in PocketBase's stored form (a space, not a "T") - `pb_hooks/lib/reports/dates.js` is the one place that builds them.
...
- **Every sales figure reads `sales.occurred_at`, never `sales.created`** - `sale_lines` reaches it through the relation (`sale.occurred_at`), since the field lives on `sales` only. A refund has no `occurred_at` of its own and keeps the date logic it already had (the net-of-refunds figures still read live `refunded_qty`, whatever day that refund happened on).
- **Stock value and every `stock` report figure except sell-through are point in time**, not a reconstruction of a past day - said so above and repeated here because it is the one place a "build for date X" request can silently mean something other than what it asks for.
- **Sell-through counts stock rows, not units** - a sealed line's original quantity is not recoverable once it has partly sold (PLAN.md: "a sale decrements qty"), so a multi-quantity row only counts as sold once it is fully gone.
...
- **The 400-day bound and its message are shared** by `GET /api/vault/reports/:key` (and its `.csv` and `audit.csv` variants) and `POST /api/vault/stats/rebuild`: `"Pick a range of up to 400 days."`
- **`daily_stats` is matched by date as a same-day range**, never text equality, for the same reason every other date field in this codebase is - PocketBase's stored form is a full timestamp, and a bare date string is not it. `upsertDayRow` (`pb_hooks/lib/reports/daily.js`) finds-or-creates this way, so `POST /api/vault/stats/rebuild` run twice over the same range never produces a second row for a day.
...
- **Dependencies**: none added. Every helper reused (`pb_hooks/lib/vaultutil.js`, `lib/audit.js`, `lib/shared/{money,loyalty,saleline}.js`) already existed.
