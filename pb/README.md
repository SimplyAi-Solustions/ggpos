...
| `lib/readers.js` | Phase 7: the SumUp Solo card reader at the counter. `config`/`isConfigured`/`saveSumupSettings` (the merchant key, code and default reader off `settings`), `returnUrlFor` (the callback URL, refused unless this instance has an https address, or is on 127.0.0.1/localhost for development), `hashToken`/`newToken`/`findByToken` (only the sha256 of a callback token is ever stored), `createCheckout` (idempotent per `sale_client_id`), `decide` (pure: what a fetched transaction means for a pending checkout, including a mismatched amount), `applyOutcome` (the write, inside its own transaction, and only while the row is still `pending`), `verify` (the transactions lookup, then the reader's own status as a fallback that can only ever fail a payment, never pay one), `cancel`, `expirePending` (the cron) and `saleRefusal` (the one wording for every way a card payment cannot pay for a sale, used by `sales.complete` before and inside its transaction). |
...
| `POST /api/vault/sumup/checkouts` | Puts an amount on the reader. Idempotent per `sale_client_id`. Rate limited to 30 a minute per caller. |
...
| `POST /api/vault/labels/queue`, `/claim`, `/:id/printed`, `/:id/failed`, `/:id/requeue` | The bulk reprint and the cross-device print queue. A claim is atomic: two devices claiming at once take different batches. |
...
price snapshot roll-up and the label queue. It pairs a reader (and refuses
...
unstick cron. It finishes on the API rules: a customer token reaches none
of the new routes, and a staff token can read a `sumup_checkouts` row but
never write one.

That section needs two things the API cannot set: `sumup_checkouts.created`
(an autodate, which the expiry cron measures from) and `callback_secret`,
which holds the sha256 of a token the server generates and never hands
out - so posting a callback the way SumUp does means putting a known
token's own hash there first. It starts its own short-lived PocketBase for
