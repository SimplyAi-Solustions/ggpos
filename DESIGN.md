...
| `ShortcutOverlay` (in `app/`) | Every keyboard shortcut, drawn as `Kbd` keys rather than described, grouped under Go to, Find and Help, with one line under them saying Esc closes any sheet, dialog or menu. `?` opens it anywhere on the counter and never while the caret is in a field, Esc closes it, and it is in the command palette for anybody who reached for the mouse. The list is `app/shortcuts.ts` itself, so a key the counter gains and this page does not cannot happen. |
| `CardPaymentSheet` (in `features/sell/`) | Waiting for the Solo reader: the amount in Anton at 28px, one sentence beside the thin ring about where the payment has got to, and Cancel. A decline says why in the reader's own words and offers Try again. Money taken on a sale that then refuses is the one state that never closes to nothing: it shows the SumUp transaction code and the two ways out, and closing it leaves the same facts as a grey line under the card amount on Sell until the sale carries them. |
| Label queue states (in `features/labels/`) | Where a job has got to is said in words under the item's title, never in a colour: "Printing on Counter PC" while another device holds it, and the printer's own sentence with a "Queue again" action once it has failed three times. The printer block above the queue is per device, not per shop, so a browser with no WebUSB says "Queued for the counter printer" in one line and shows no controls at all. |
